...
def get_monday_webtoons_info():
    url = "https://comic.naver.com/webtoon/weekday?weekday=mon"
    response = requests.get(url)
    response.raise_for_status()  # HTTP 에러 발생 시 예외를 발생시킴
    soup = BeautifulSoup(response.text, 'html.parser')

    webtoon_list = soup.select('#content > div.list_area.daily_img > ul > li')
    webtoon_info_list = []

    for webtoon in webtoon_list:
        try:
            # 썸네일 이미지 주소
            thumbnail_element = webtoon.select_one('div.thumb > a > img')
            thumbnail_url = thumbnail_element['src'] if thumbnail_element else None

            # 타이틀
            title_element = webtoon.select_one('div.info > h5 > a')
            title = title_element.text.strip() if title_element else None

            # 작가명
            author_element = webtoon.select_one('div.info > p > a')
            author = author_element.text.strip() if author_element else None

            # 평점
            rating_element = webtoon.select_one('div.rating_type > strong')
            ...
            print(f"Error processing a webtoon: {e}")

    return webtoon_info_list

if __name__ == "__main__":
    monday_webtoons = get_monday_webtoons_info()
    for info in monday_webtoons:
        print(f"썸네일: {info['thumbnail_url']}")
